...
class PlayState extends MusicBeatState
{
	...
	public static var Strictness:Int = 95;
	public static var HSEnabled:Bool = false;
	public static var LightBoi:Bool = true;
	...
	public static var generate:Array<Dynamic> = [
		['LEFT','DOWN','UP','RIGHT'],
		['LEFT','DOWN','RIGHT','LEFT','UP','RIGHT'],
		['LEFT','DOWN','UP','RIGHT','SPACE', 'LEFT','DOWN','UP','RIGHT'], //sh*t
		['LEFT','DOWN','UP','RIGHT', 'LEFT','DOWN','UP','RIGHT']
	];

	public static var curK:Int = 0;
	public static var curC:Int = 4;
	...
	override public function create()
	{
		...
		Strictness = Settings.Get("Input Strictness");
		...
		Deez = SONG.keySet;
		keyC = Note.alsoVuk[Deez];

		curK = Deez;
		curC = Note.alsoVuk[curK];
		
		//4k
		Note.BoundKeys[0] = Settings.Get("4K Controls");
		//6k
		Note.BoundKeys[1] = Settings.Get("6K Controls");
		//9k
		Note.BoundKeys[2] = Settings.Get("9K Controls");
		//8k
		Note.BoundKeys[3][0] = Settings.Get("9K Controls")[0];
		Note.BoundKeys[3][1] = Settings.Get("9K Controls")[1];
		Note.BoundKeys[3][2] = Settings.Get("9K Controls")[2];
		Note.BoundKeys[3][3] = Settings.Get("9K Controls")[3];
		Note.BoundKeys[3][4] = Settings.Get("9K Controls")[5];
		Note.BoundKeys[3][5] = Settings.Get("9K Controls")[6];
		Note.BoundKeys[3][6] = Settings.Get("9K Controls")[7];
		Note.BoundKeys[3][7] = Settings.Get("9K Controls")[8];

		Main.wm.poggars = SONG.song.toUpperCase() + ". " + curC + "K. Round Engine";
		...
		if (Downscroll) healthBarBG.y = FlxG.height * 0.1;
		...
		healthBar.createFilledBar(0xFFFF0000, FlxColor.fromRGB(0,140,255));
		// healthBar
		add(healthBar);

		scoreTxt = new FlxText(0, healthBarBG.y + 30, 0, "", 23);
		scoreTxt.setFormat("assets/fonts/vcr.ttf", 23, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.screenCenter(X);
		if (Settings.Get("Show HUD")) add(scoreTxt);
		
		timeTxt = new FlxText(20, FlxG.height - 40, 0, "aaaAAA 111", 30);
		timeTxt.setFormat("assets/fonts/vcr.ttf", 30, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timeTxt.scrollFactor.set();
		timeTxt.alpha = 0;
		if (Settings.Get("Show HUD")) add(timeTxt);

		iconP1 = new HealthIcon(SONG.player1, true);
		iconP1.y = healthBar.y - (iconP1.height / 2);
		if(Settings.Get("Show HUD")) add(iconP1);

		iconP2 = new HealthIcon(SONG.player2, false);
		iconP2.y = healthBar.y - (iconP2.height / 2);
		if(Settings.Get("Show HUD")) add(iconP2);
	...
	function startSong():Void
	{
		...
		FlxTween.tween(timeTxt, {alpha: 1}, 1, {ease: FlxEase.sineOut});
	}

	var debugNum:Int = 0;

	function posToStrumTime(pos:Float):Float
	{
		return FlxMath.remapToRange(pos, 0, SONG.highestNote, 0, SONG.highestNote * Conductor.stepCrochet);
	}

	var oldChartSystem:Bool = true;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var Delay = Settings.Get("Audio Delay");
		// NEW SHIT
		...
		for(i in 0...keyC){ //so when the song loads, it secretly hits 4 notes to cache it, this way the game doesn't lag when you hit the first notes.
			var cacheNote:Note = new Note(0, i, null);
			cacheNote.mustPress = true;
			goodNoteHit(cacheNote, true);
		}
		
		trace(SONG.legacyChart);
		
		if(!SONG.legacyChart) //idk why I have to do this?
			oldChartSystem = SONG.legacyChart;
		...
		for (section in noteData)
		{
			...
			{
				var ownMania:Int = Std.int(songNotes[3]) == 0 ? Deez : Std.int(songNotes[3]) - 1;
				trace(ownMania);

				var daStrumTime:Float = oldChartSystem ? songNotes[0] : posToStrumTime(songNotes[0]);
				var daNoteData:Int = Std.int(songNotes[1] % Note.alsoVuk[ownMania]);
				...
				var swagNote:Note = new Note(daStrumTime + Delay, daNoteData, oldNote, Std.int(songNotes[3]));
				swagNote.sustainLength = songNotes[2];
				swagNote.scrollFactor.set(0, 0);

				if (songNotes[1] > Note.psC - 1)
				{
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note((daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet) + Delay, daNoteData, oldNote, true, Std.int(songNotes[3]));
		...
		for (i in 0...keyC)
		{
			...
			switch (curStage)
			{
				...
				default:
					...
					babyArrow.setGraphicSize(Std.int(babyArrow.width * Note.vuk[Deez]));
			...
			babyArrow.animation.play('confirm'); // hopefully this will cache this animation.
	...
	public function centerStrum(strum:Int)
	{
	...
	{
		return Std.string(TimeFormatter.FormatThis(FlxG.sound.music.length - Conductor.songPosition));
	}
	
	public function dadNoteHit(daNote:Note){
		...
		dad.playAnim('sing' + bruj[curK][daNote.noteData] + altAnim, true);
	...
	public function regerateArrows(player:Int, man:Int):Void
	{
		var darde = Note.alsoVuk[man];
		for (i in 0...darde)
		{
			// FlxG.log.add(i);
			var babyArrow:FlxSprite = new FlxSprite(0, strumLine.y);
			...
			switch (curStage)
			{
				...
				default:
					...
					babyArrow.antialiasing = Settings.Get("Antialiasing");
					babyArrow.setGraphicSize(Std.int(babyArrow.width * Note.vuk[man]));
			...
			centerStrum(player != 1 ? i : i + darde);

			changingMania = false;
		}
	}

	var changingMania:Bool = false;
	public var tooLazyToUseFlxTimer:Float = 0;

	public function changeManiaMidSong(garGa:Int){
		changingMania = true;
		strumLineNotes.forEach(function(dada:FlxSprite){
			FlxTween.tween(dada, {alpha: 0}, 0.2, {ease: FlxEase.sineOut});
		});
		FlxTween.tween(this, {tooLazyToUseFlxTimer: 3}, 0.2, {ease: FlxEase.sineOut, onComplete: function(twn:FlxTween){
			...
			regerateArrows(0, garGa);
			regerateArrows(1, garGa);
			tooLazyToUseFlxTimer = 0;

			/*switch (garGa)
			{
				case 0:
					Main.wm.poggars = SONG.song.toUpperCase() + ". 4K. Round Engine";
					Note.BoundKeys[garGa] = Settings.Get("4K Controls");
				case 1:
					Main.wm.poggars = SONG.song.toUpperCase() + ". 6K. Round Engine";
					Note.BoundKeys[garGa] = Settings.Get("6K Controls");
				case 2:
					Main.wm.poggars = SONG.song.toUpperCase() + ". 9K. Round Engine";
					Note.BoundKeys[garGa] = Settings.Get("9K Controls");
				case 3:
					Main.wm.poggars = SONG.song.toUpperCase() + ". 8K. Round Engine";
					Note.BoundKeys[garGa][0] = Settings.Get("9K Controls")[0];
					Note.BoundKeys[garGa][1] = Settings.Get("9K Controls")[1];
					Note.BoundKeys[garGa][2] = Settings.Get("9K Controls")[2];
					Note.BoundKeys[garGa][3] = Settings.Get("9K Controls")[3];
					Note.BoundKeys[garGa][4] = Settings.Get("9K Controls")[5];
					Note.BoundKeys[garGa][5] = Settings.Get("9K Controls")[6];
					Note.BoundKeys[garGa][6] = Settings.Get("9K Controls")[7];
					Note.BoundKeys[garGa][7] = Settings.Get("9K Controls")[8];
			}*/

			Main.wm.poggars = SONG.song.toUpperCase() + ". " + Note.alsoVuk[garGa] + "K. Round Engine";

			strumLineNotes.cameras = [camStrums];
		}});
	}

	var dontFireTwice:Bool = false;
	...
	override public function update(elapsed:Float)
	{
		...
		var FCText:String = "undefined lol";
		
		if (MissCount == 0) FCText = ["Lmao", "(bad) FC", "FC", "GFC", "SFC"][FCrate];
		else if (MissCount < 10) FCText = "SDCB";
		else if (MissCount >= 10) FCText = "Clear";
 		
		if (Accuracy < 0 && FCrate == 0)
			scoreTxt.text = "No scores yet";
		else
			scoreTxt.text = "Score:" + songScore + " | Misses: " + MissCount + " | Accuracy: " + (Math.round(Accuracy * 100) / 100) + "% - " + FCText;
		...
		if (timeTxt.text == "0:00" && !dontFireTwice){
			FlxTween.tween(timeTxt, {alpha: 0}, 0.2, {ease: FlxEase.sineOut});
			dontFireTwice = true;
		}
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			{
				var MODX = dad.getMidpoint().x + 150;
				var MODY = dad.getMidpoint().y - 100;
				...
				camFollow.setPosition(FlxMath.lerp(camFollow.x, MODX, CamSpeed), FlxMath.lerp(camFollow.y, MODY, CamSpeed));
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
				...
				var MODX = boyfriend.getMidpoint().x - 100;
				var MODY = boyfriend.getMidpoint().y - 100;
		...
		if (camZooming)
		{
			...
			camStrums.zoom = FlxMath.lerp(1, camStrums.zoom, 0.95);
		...
		var roundedSpeed:Float = FlxMath.roundDecimal(SONG.speed, 2); //honestly dont know what tf this is for but whatever
		if (unspawnNotes[0] != null)								  // CTRL + V
		{
			var time:Float = 1500;
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (((!daNote.isSustainNote && daNote.wasGoodHit) || (daNote.isSustainNote && Math.abs(daNote.y - strumLine.y) <= 36)) && daNote.mustPress && HSEnabled)
				{
					...
					boyfriend.playAnim('sing' + bruj[curK][daNote.noteData], true);
					
					lmaoImDumb(daNote.noteData + curC, daNote.isSustainNote);
				...
				var marX:Float = 0;
				var marY:Float = 1;
				if (daNote.mustPress && !changingMania){
					marX = Math.cos((playerStrums.members[daNote.noteData % curC].angle + 90) * Math.PI / 180);
					marY = Math.sin((playerStrums.members[daNote.noteData % curC].angle + 90) * Math.PI / 180);
				} else if(!changingMania) {
					marX = Math.cos((strumLineNotes.members[daNote.noteData % curC].angle + 90) * Math.PI / 180);
					marY = Math.sin((strumLineNotes.members[daNote.noteData % curC].angle + 90) * Math.PI / 180);
				}

				var completeGarbage:Float = 0;
		...
		if (!inCutscene && !HSEnabled && !changingMania)
			pooplol();
		else if (!inCutscene && HSEnabled){
	...
	var prevDiff :Float = 0;
	
	var scrores:Array<Int> = [];

	private function popUpScore(strumtime:Float, cache:Bool = false):Void
	{
		var noteDiff:Float = Math.abs((strumtime - IND) - Conductor.songPosition);
		...
		if(!MiddleScroll && !cache){
			if (noteDiff > Conductor.safeZoneOffset * 0.9)
			{
				daRating = 'shit';
				if (Accuracy < 0) Accuracy = 20;
				if (FCrate > 1) FCrate = 1;
				score = 50;
			}
			...
			else if (noteDiff > Conductor.safeZoneOffset * 0.2)
			{
				...
				if (FCrate > 3) FCrate = 3;
				score = 200;
			}
			
			if (FCrate <= 0) FCrate = 4;
			
			if (Accuracy < 0) Accuracy = 100;
			
			if (noteDiff < prevDiff && score >= 200)
				Accuracy += 1.7;
			else if(Math.abs(noteDiff - prevDiff) >= 2)
				Accuracy -= 1.45;
			...
			if (scrores.length > 100){ // can't be too memory intensive
				scrores = [Std.int(Accuracy * 3.5)];
				trace('CLEARED!');
			}
			
			scrores.push(score);
			
			var fakeAccuracy:Int = 0;
			for (i in 0...scrores.length)
				fakeAccuracy += scrores[i];
			if (!cache) Accuracy = (fakeAccuracy / scrores.length) / 3.5;
			if (cache) scrores = [];
			if (cache) FCrate = 0;
		}
		...
		rating.screenCenter();
		rating.x = (coolText.x - 40) + Settings.Get("Coff")[0];
		...
		if (!curStage.startsWith('school'))
		{
			...
			comboSpr.antialiasing = Settings.Get("Antialiasing");
		}
		else
		{
		...
		for (i in seperatedScore)
		{
			...
			numScore.x = (coolText.x + (43 * daLoop) - 90) + Settings.Get("Coff")[0];
			numScore.y += 80 - Settings.Get("Coff")[1];
	...
	public function pooplol(){
		if (FlxG.keys.anyJustPressed(Note.BoundKeys[curK])){
			handleInput();
		}
		for (I in 0...Note.BoundKeys[curK].length){
	...
	function noteMiss(direction:Int = 1, Strumtime:Float = -1, forceA:Int = 0):Void
	{
		if (!boyfriend.stunned && !HitNotes.contains(Std.string(Strumtime)))
		{
			MissCount += 1;
			if (!MiddleScroll) Accuracy -= 5;
			else {
				scrores.push(forceA);
				var fakeAccuracy:Int = 0;
				for (i in 0...scrores.length)
			...
			boyfriend.playAnim('sing' + bruj[curK][direction] + 'miss', true);
		}
	}
	...
	function goodNoteHit(note:Note, cache:Bool = false):Void
	{
		if (!note.wasGoodHit)
		{
			if (!note.isSustainNote)
			{
				popUpScore(note.strumTime, cache);
				if(!cache) combo += 1;
			}

			if (note.noteData >= 0 && !cache)
				health += 0.023;
			else
				health += 0.004;
			...
			boyfriend.playAnim('sing' + bruj[curK][note.noteData % curC], true);
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'milf' && curBeat >= 168 && curBeat <= 200 && camZooming && FlxG.camera.zoom < 1.35 && Settings.Get("Camera Zooms"))
		{
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && totalBeats % 4 == 0 && Settings.Get("Camera Zooms"))
		{
			FlxG.camera.zoom += 0.015;
			camStrums.zoom += 0.03;
			camHUD.zoom += 0.03;
